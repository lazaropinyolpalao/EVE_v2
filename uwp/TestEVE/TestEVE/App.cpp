...
void App::Load(Platform::String^ entryPoint)
{
	if (m_main == nullptr)
	{
		...
		m_main = std::unique_ptr<TestEveMain>(new TestEveMain(m_deviceResources));
		//m_deviceResources
		//m_deviceResources->GetSwapChain()->GetHwnd(&hwnd);
		//m_main = std::unique_ptr<TestEveMain>(new TestEveMain(m_deviceResources, hwnd));
	}
}
