...
struct Audio {
	...
	*/
	bool loadWAVFile(std::string filepath);

	/**
	* @brief Inits a WAV file to the audio resource. No need for ALUT or ALURE with this
	* 
	...
	*/
	bool initWAVFile(const std::string filename, ALuint* buffer, ALsizei* size, ALsizei* frequency, ALenum* format);
